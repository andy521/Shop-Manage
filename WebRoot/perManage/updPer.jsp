...
<form action="perManage!updSave.action" method="post" class="form form-horizontal responsive" id="demoform">
			<input type="hidden" name="updUsers.usersid" id="updUsers.usersid" value="${usersid}">
			<div class="row cl">
			<label class="form-label col-xs-2">登录名称：</label>
			<div class="formControls col-xs-5">
				<input type="text" required   placeholder="请输入登录名称！" datatype="*" nullmsg="登录名称不能为空!" value='${username}'  class="input-text radius" name="updUsers.username" id="updUsers.username">
			</div>
		...
		<div class="row cl">
			...
			<div class="formControls col-xs-5">
				<input type="text"   placeholder="用户密码为空则为不修改！"    class="input-text radius" name="updUsers.password" id="updUsers.password" >
			</div>
			<div class="col-xs-5"> </div>
		</div>
        ...
        <div class="row cl">
			...
			<div class="formControls col-xs-5">
				<input type="text" required  placeholder="请输入真实姓名!" datatype="*" nullmsg="真实姓名不能为空!" value="${realname}"     class="input-text radius"  name="updUsers.realname" id="updUsers.realname" >
			</div>
			<div class="col-xs-5"> </div>
		</div>
        ...
        <div class="row cl">
			...
			<div class="formControls col-xs-5">
				<input type="text" required  placeholder="请输入联系方式!" datatype="tel" nullmsg="联系方式不能为空!" errormsg="请输入正确的联系方式！"  value="${tel}" class="input-text radius"  name="updUsers.tel" id="updUsers.tel" >
			</div>
			<div class="col-xs-5"> </div>
		</div>
		...
		<div class="row cl">
			...
			<div class="formControls col-xs-5"> 
			<input type="text" required  placeholder="请输入身份证号码!" datatype="idcard" nullmsg="身份证号码不能为空!" errormsg="请输入正确的身份证号码！" value="${idcard}" class="input-text radius"  name="updUsers.idcard" id="updUsers.idcard" >
				</div>
			<div class="col-xs-5"> </div>
		</div>
		<div class="row cl">
			...
			<div class="formControls col-xs-5">
				...
				<input type="text" required  placeholder="请输入出生日期!" datatype="date" nullmsg="出生日期不能为空!" value="<s:date format="yyyy-MM-dd" name="birthday"/>" onClick="WdatePicker()" class="input-text radius Wdate"  name="updUsers.birthday" id="updUsers.birthday" >
			</div>
			<div class="col-xs-5"> </div>
		</div>
		<div class="row cl">
			...
			<div class="formControls col-xs-5"> 
				<input type="text" required  placeholder="请输入入职时间!" datatype="*" nullmsg="入职时间不能为空!"  onClick="WdatePicker()" value="<s:date format="yyyy-MM-dd" name="positionBirthday"/>" class="input-text radius Wdate"  name="updUsers.positionBirthday" id="updUsers.positionBirthday" >
				 </div>
			<div class="col-xs-5"> </div>
		</div>
...
<s:iterator value="updUsers" id="a">
<script>
 $("#shopid").val("<s:property value="#a.shop.shopid"/>");
 $("#shopid").change();

 var test = setTimeout(function(){
	 $("#alongid").val("<s:property value="#a.along.alongid"/>");
	 $("#headerid").val("<s:property value="#a.users.usersid"/>");
	 $("#positionid").val("<s:property value="#a.position.positionid"/>");
 },200);
 </script>
 </s:iterator>
</body>
