...
<![endif]-->
 ...
 <body>
  <div class="container">
  <form action="along!selalong.action" method="post" name="sel">
  ...
  <table class="table table-border table-bordered table-hover table-bg mt-10">
		...
		<thead>
			<tr>
				<th class="col1">序号</th>
				<th class="col2">部门名称</th>
				<th class="col3">所属店铺</th>
				<th class="col3">操作</th>
			</tr>
		</thead>
		<tbody>
		<s:iterator value="alongs" status="n" id="s">
			<tr>
			
			<th class="col1 text-c"><s:property value="#n.count"/></th>
			<form action="" method="post" name="<s:property value="#s.alongid"/>" id="<s:property value="#s.alongid"/>">
			<input type="hidden" name="along.alongid" value="<s:property value="#s.alongid"/>">
			<td class="col2 text-c">
			<input type="text" required class="input-text radius size-M" placeholder="" name="along.alongname" id="<s:property value="#s.alongid"/>alongname" value="<s:property value="#s.alongname"/>">
			</td>
			<td class="col2 text-c">
			<span class="formControls col-xs-5 select-box radius">
			<select class="select radius" size="1" name="shopid" id="shopid">
			...
			<td class="col2 text-c">
			<input class="btn radius btn-warning" type="button" value="修改部门" onclick="upd('<s:property value="#s.alongid"/>');">
			<input class="btn btn-danger radius" type="button" value="删除部门" onclick="del('<s:property value="#s.alongid"/>');">
			</td>
			</form>
			</tr>
			...
			<tr>
			<form action="along!addSave.action" method="post" onsubmit="return add1()" name="add">
			<th class="col1 text-c">自动添加</th>
			<td class="col2 text-c">
			<input type="text" required class="input-text radius size-M" placeholder="" name="along.alongname" id="addname" value="">
			</td>
			<td class="col2 text-c">
			<span class="formControls col-xs-5 select-box radius">
			<select class="select radius" size="1" name="shopid" id="shopid">
			...
			<td class="col2 text-c">
			<input class="btn btn-success radius"  type="submit"  value="添加部门">
			</td>
			</form>
			</tr>
		</tbody>
...
function upd(a) {
	document.getElementById(a).action='along!updSave.action';
	var x=document.getElementById(a+"alongname").value;
	if(x!="")
...
function del(a) {
	document.getElementById(a).action='along!delSave.action';
	document.getElementById(a).submit();
}
